from utils import CriticsDataCleaner, MoviesDataCleaner, BestPictureOscarsDataCleaner, AnyWinOscarsDataCleaner
...
if end_rows > start_rows:
    raise Exception("Number of rows increased after merging with movie title. This is unexpected.")
if end_rows < start_rows:
    print(f"{start_rows - end_rows} rows have been dropped because they did not match with a movie title.")
...
if False: # TODO: give this option as a python arg? 
    best_picture_data = best_picture_data.merge(critics_data, on = 'movie_title', how='left')
